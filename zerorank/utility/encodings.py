...
def main(data : str,
    ...
    q_embeddings = np.concatenate(model.encode(frame['query'].tolist(), show_progress_bar=True, batch_size=batch_size, convert_to_numpy=True), axis=0)
    np.save(join(output, 'q_embeddings.npy'), q_embeddings)
    del q_embeddings
    a_embeddings = np.concatenate(model.encode(frame['text_a'].tolist(), show_progress_bar=True, batch_size=batch_size, convert_to_numpy=True), axis=0)
